...
//---------------------------------------------------------------------------------------------
    ...
    let newTitle = '';
    // Verschiedene Titel basierend auf der Sektion
    if (sectionId === 'login-section') {
        newTitle = 'Tafel Sterkrade Login';
    } else if (sectionId === 'formular-section') {
        newTitle = 'Tafel Sterkrade Anmeldung';
    } else {
        newTitle = 'Tafel Sterkrade App'; // Fallback
    }
...
//---------------------------------------------------------------------------------------------
    ...
    // 2. Sichtbarkeit der Sektionen steuern
...
//---------------------------------------------------------------------------------------------
async function showFormAndLoadData(username) {
//---------------------------------------------------------------------------------------------
  showSection('formular-section');

  try {
    // Warten, bis das Formular geladen und das Lade-Overlay ausgeblendet ist
    await initFormPage(username); 

    showOverlayNews();

  } catch (error) {
    console.error("Fehler beim Anzeigen des Formulars oder Laden der Daten:", error);
  }
